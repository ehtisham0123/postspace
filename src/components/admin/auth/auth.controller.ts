...
import { Session, SessionModel } from "../../../models/Admin";
...
export const checkSession = async (
  ...
  try {
    ...
    const { userName, privilege } = <AdminDocument>req.user;
...
export const login = async (
  ...
  try {
    Logger.info("login");
    const { adminId, userName, privilege, _id } = <AdminDocument>req.user;
...
export const logout = async (
  ...
  try {
    Logger.info(req.user);
    const { _id } = <AdminDocument>req.user;
...
export const refreshToken = async (
  ...
  try {
    ...
    if (!admin) throw new Error("Invalid Token admin not found");
    ...
    const token = getToken({ _id: userId, type: admin.privilege });
